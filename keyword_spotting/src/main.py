...
def parse_args():
    ...
    parser.add_argument("--ema-start-epoch", type=int, default=-1, help="Epoch to begin EMA updates (-1 = epochs // 2)")
...
def main():
    ...
    epochs = args.epochs
    ema_start_epoch = args.ema_start_epoch if args.ema_start_epoch >= 0 else epochs // 2
    ...
    print(f"EMA: {args.ema}" + (f" (decay={args.ema_decay}, start_epoch={ema_start_epoch})" if args.ema else ""))
    ...
    ema_state = {k: v.detach().clone().float() for k, v in model.state_dict().items()} if args.ema else None
    ...
    for epoch in range(1, epochs + 1):
        ...
        if ema_state is not None and epoch >= ema_start_epoch:
            for k, v in model.state_dict().items():
                ema_state[k].mul_(args.ema_decay).add_(v.detach().float(), alpha=1 - args.ema_decay)
    ...
    if ema_state is not None:
        ema_dtype = next(model.parameters()).dtype
        model.load_state_dict({k: v.to(dtype=ema_dtype) for k, v in ema_state.items()})
        evaluate(model, val_loader, criterion, device, split_name="EMA Val")
        evaluate(model, test_loader, criterion, device, split_name="EMA Test")
